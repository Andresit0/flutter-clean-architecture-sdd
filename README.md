...
- **Dependency policy** — the Flutter SDK version is pinned once in `pubspec.yaml` (`environment.flutter: 3.47.4`) and CI reads it via `flutter-version-file`, so there is no hardcoded version in the workflows. Flutter 3.47.4 requires `intl` (`^0.20.3`, forced by `flutter_localizations`) and pins `test_api` (0.7.12), `matcher` (0.12.20), `meta` (1.19.0) and `vector_math` (2.4.2); never force-bump them (breaks `flutter pub get`). Dependabot ignores `intl`/`test`/`freezed-major` (see `.github/dependabot.yml`), but does **not** honor `ignore` inside grouped updates (dependabot-core #10122/#13213) — the constraints plus CI are the real guard. The codegen toolchain stays on analyzer 12 / freezed 3.2.6 with Dart language version 3.12 because Dart LV 3.13 rejects the `final` parameters its generated code emits; adopting freezed 4 (analyzer 13) is tracked in issue #62. See `MD/APP_COMMANDS.md` and `.github/REQUIRED_CHECKS.md`.
