...
- The codegen toolchain stays on analyzer 12 / freezed 3.2.6 with Dart language version 3.12 (`environment.sdk: ^3.12.0`): Dart LV 3.13 rejects the `final` parameters freezed 3.2.6 emits. Adopting freezed 4 (analyzer 13) requires a language-version bump and is tracked in issue #62.
- Dependabot ignores `intl`, `test` and `freezed` (semver-major). Dependabot reads `.github/dependabot.yml` from the **default branch (`main`)**.
